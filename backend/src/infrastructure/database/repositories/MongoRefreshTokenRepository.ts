...
export class MongoRefreshTokenRepository implements IRefreshTokenRepository {
    ...
    async findByUserId(userId: string): Promise<RefreshToken[]> {
        try {
            ...
            Logger.error('Error finding refresh tokens by user ID', { userId, error });
            throw error;
        }
    }
    ...
    async save(refreshToken: RefreshToken): Promise<void> {
        try {
            ...
            Logger.info('Refresh token saved successfully', { tokenId: refreshToken.getId() });
        } catch (error) {
            Logger.error('Error saving refresh token', { tokenId: refreshToken.getId(), error });
            throw error;
        }
    }
    ...
    async deleteByUserId(userId: string): Promise<void> {
        try {
            ...
            Logger.error('Error deleting refresh tokens by user ID', { userId, error });
            throw error;
        }
    }
    ...
    private toPersistence(refreshToken: RefreshToken): Partial<IRefreshTokenDocument> {
        return {
