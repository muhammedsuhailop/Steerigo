...
let EmailService = class EmailService {
    ...
    async sendVerificationOtp(email, otp, userName) {
        try {
            ...
            Logger_1.Logger.info("Verification OTP email sent successfully", { email });
        }
        catch (error) {
            Logger_1.Logger.error("Failed to send verification OTP email", { email, error });
    ...
    async sendPasswordResetOtp(email, otp, userName) {
        try {
            ...
            await this.transporter.sendMail(mailOptions);
            Logger_1.Logger.info("Password reset OTP email sent successfully", { email });
        }
        catch (error) {
            Logger_1.Logger.error("Failed to send password reset OTP email", { email, error });
    ...
    async sendWelcomeEmail(email, userName) {
        try {
            ...
            await this.transporter.sendMail(mailOptions);
            Logger_1.Logger.info("Welcome email sent successfully", { email });
        }
        catch (error) {
      ...
      <html>
        ...
        <body>
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <p>This code will expire in ${Math.floor(parseInt(process.env.OTP_TTL_SECONDS || "300") / 60)} minutes.</p>
