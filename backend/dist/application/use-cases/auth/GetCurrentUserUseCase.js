...
let GetCurrentUserUseCase = class GetCurrentUserUseCase {
    ...
    async execute(dto) {
        try {
            Logger_1.Logger.info("Get current user started", { userId: dto.getUserId() });
            const user = await this.userRepository.findById(dto.getUserId());
            if (!user) {
                Logger_1.Logger.warn("Get current user failed - user not found", {
            ...
            Logger_1.Logger.error("Get current user use case error", {
                userId: dto.getUserId(),
