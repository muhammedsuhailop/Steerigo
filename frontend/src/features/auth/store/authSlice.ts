...
export const initializeAuth = createAsyncThunk(
  ...
  async (_, { dispatch }) => {
    ...
    // If we have tokens but no user, try to fetch current user
...
//     try {
//       const response = await GoogleAuthService.initiateGoogleLogin();
//       if (response.success && response.data?.authUrl) {
//         // Redirect to Google OAuth
//         window.location.href = response.data.authUrl;
//         return response;
//       }
//       return rejectWithValue(response.message || 'Failed to initiate Google login');
//     } catch (error: any) {
//       return rejectWithValue(error.message || 'Google authentication initiation failed');
//     }
//   }
// );
export const initiateGoogleAuth = createAsyncThunk(
...
//     //   return response;
//     } catch (error: any) {
//       return rejectWithValue(error.message || 'Google callback handling failed');
//     }
//   }
// );
...
const mapDecodedToUser = (decoded: DecodedJwt): User => ({
  id: decoded.id,
  email: decoded.email,
  name: decoded.name?.split(" ")[0] || "",
  //   lastName: decoded.name?.split(' ').slice(1).join(' ') || '',
  role: (decoded.role as any).toUpperCase(),
